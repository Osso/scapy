...
def do_graph(graph, prog=None, format=None, target=None, type=None, string=None, options=None):
    ...
    if format is None:
        if WINDOWS:
            # use common format to make sure a viewer is installed
            format = "png"
        else:
            format = "svg"
    if string:
...
sync: do not bufferize writes to the capture file
    ...
    def _write_packet(self, packet, sec=None, usec=None, caplen=None, wirelen=None):
        ...
        self.f.write(
            struct.pack(self.endian+"IIII", sec, usec, caplen, wirelen))
        self.f.write(packet)
        if self.sync:
            self.f.flush()
    ...
    def _write_header(self, pkt):
        if self.linktype == None:
            try:
                ...
                warning(
                    "PcapWriter: unknown LL type for %s. Using type 1 (Ethernet)" % pkt.__class__.__name__)
                self.linktype = 1
