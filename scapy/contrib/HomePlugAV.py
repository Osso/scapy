...
class GetDeviceVersion(Packet):
    ...
    fields_desc = [ByteEnumField("Status", 0x0, StatusCodes),
                   ...
                   StrLenField(
                       "DeviceVersion", "NoVersion\x00", length_from=lambda pkt: pkt.VersionLen),
                   StrLenField("DeviceVersion_pad", "\xcc\xcc\xcc\xcc\xcc" +
                               "\x00"*59, length_from=lambda pkt: 64-pkt.VersionLen),
...
class ReadMACMemoryConfirmation(Packet):
    ...
    fields_desc = [ByteEnumField("Status", 0x00, ReadMACStatus),
                   ...
                   FieldLenField(
                       "MACLen", None, length_of="MACData", fmt="<H"),
                   StrLenField(
                       "MACData", "\x00", length_from=lambda pkt: pkt.MACLen),
                   ]
...
class ReadModuleDataConfirmation(Packet):
    ...
    fields_desc = [ByteEnumField("Status", 0x0, StatusCodes),
                   ...
                   FieldLenField(
        "DataLen", None, count_of="ModuleData", fmt="<H"),
        LEIntField("Offset", 0x00000000),
...
class HomePlugAV(Packet):
    ...
    fields_desc = [MACManagementHeader,
                   # Fragmentation Field
                   ConditionalField(
                       XShortField("FragmentInfo", 0x0), FragmentCond),
                   VendorMME]
