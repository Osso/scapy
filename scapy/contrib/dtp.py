...
class DTPDomain(DtpGenericTlv):
    ...
    fields_desc = [ShortField("type", 1),
                   ...
                   StrLenField(
                       "domain", "\x00", length_from=lambda pkt:pkt.length - 4)
                   ]
...
class DTPStatus(DtpGenericTlv):
    ...
    fields_desc = [ShortField("type", 2),
                   ...
                   StrLenField(
                       "status", "\x03", length_from=lambda pkt:pkt.length - 4)
                   ]
...
class DTPType(DtpGenericTlv):
    ...
    fields_desc = [ShortField("type", 3),
                   ...
                   StrLenField(
                       "dtptype", "\xa5", length_from=lambda pkt:pkt.length - 4)
                   ]
...
def negotiate_trunk(iface=conf.iface, mymac=str(RandMAC())):
    ...
    p = Dot3(src=mymac, dst="01:00:0c:cc:cc:cc")/LLC()/SNAP() / \
        DTP(tlvlist=[
            DTPDomain(), DTPStatus(), DTPType(), DTPNeighbor(neighbor=mymac)])
    sendp(p)
