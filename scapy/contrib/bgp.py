...
class BGPPathAttribute(Packet):
    ...
    fields_desc = [
        ...
        ByteEnumField("type", 1, {1: "ORIGIN", 2: "AS_PATH", 3: "NEXT_HOP", 4:
                                  "MULTI_EXIT_DISC", 5: "LOCAL_PREF", 6: "ATOMIC_AGGREGATE", 7: "AGGREGATOR"}),
...
class BGPUpdate(Packet):
    ...
    fields_desc = [
        ...
        FieldListField("nlri", [], BGPIPField("", "0.0.0.0/0"), length_from=lambda p:
                       p.underlayer.len - 23 - p.tp_len - p.withdrawn_len),  # len should be BGPHeader.len
    ]
...
class BGPNotification(Packet):
    ...
    fields_desc = [
        ByteEnumField("ErrorCode", 0, {1: "Message Header Error", 2: "OPEN Message Error", 3:
                                       "UPDATE Messsage Error", 4: "Hold Timer Expired", 5: "Finite State Machine", 6: "Cease"}),
        ByteEnumField(
            "ErrorSubCode", 0, {1: "MessageHeader", 2: "OPENMessage", 3: "UPDATEMessage"}),
        LongField("Data", 0),
    ]
