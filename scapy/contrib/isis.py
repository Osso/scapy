...
class ISIS_ExtendedIpPrefix(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(FieldLenField("subtlvslen", None, length_of=lambda x:
                                       x.subtlvs, fmt="B"), lambda pkt: pkt.subtlvindicator == 1),
...
class ISIS_ExtendedIsNeighbourEntry(Packet):
    ...
    fields_desc = [ISIS_NodeIdField("neighbourid", "0102.0304.0506.07"),
                   ...
                   FieldLenField(
                       "subtlvslen", None, length_of="subtlvs", fmt="B"),
...
class ISIS_IsReachabilityTlv(ISIS_GenericTlv):
    ...
    fields_desc = [
        ...
        ByteField("virtual", 0),
        PacketListField(
            "neighbours", [], ISIS_IsReachabilityEntry, count_from=lambda x: (x.len - 1) / 11)
    ]
...
class _ISIS_PduLengthField(FieldLenField):
    ...
    def __init__(self):
        FieldLenField.__init__(
            self, "pdulength", None, length_of="tlvs", adjust=lambda pkt, x: x + pkt.underlayer.hdrlen)
...
class _ISIS_LSP_Base(_ISIS_PduBase):
    fields_desc = [
        ...
        XShortField("checksum", None),
        FlagsField(
            "typeblock", 0x03, 8, ["L1", "L2", "OL", "ADef", "ADel", "AExp", "AErr", "P"]),
        _ISIS_TlvListField()
    ]
