...
# Copyright (C) 2015 Alessio Deiana <adeiana@gmail.com>
# 2015 Alexis Sultan <alexis.sultan@sfr.com>
##
...
class GTPHeader(Packet):
    ...
    def dispatch_hook(cls, _pkt=None, *args, **kargs):
        if _pkt and len(_pkt) >= 1:
            if (struct.unpack("B", _pkt[0])[0] >> 5) & 0x7 == 1:
                return gtp_v1.GTPHeader
        return GTPHeader


class IE_IPv4(gtp_v1.IE_Base):
    name = "IE IPv4"
...
class IE_EPSBearerID(gtp_v1.IE_Base):
...
class IE_NotImplementedTLV(gtp_v1.IE_Base):
...
class IE_RecoveryRestart(gtp_v1.IE_Base):
...
class APN_Field(StrFixedLenField):

    def i2repr(self, pkt, x):
        return x


class IE_APN(gtp_v1.IE_Base):
    name = "IE APN"
    fields_desc = [ByteEnumField("ietype", 1, IEType),
                   ShortField("length",  None),
                   BitField("CR_flag", 0, 4),
...
class PCO_Secondary_DNS(Packet):
...
class PCO_Secondary_NBNS(Packet):
...
class PCO_P_CSCF_IPv6_Address_Request(Packet):
...
class PCO_DNS_Server_IPv6(Packet):
...
class PCO_IP_Allocation_via_NAS(Packet):
...
class PCO_P_CSCF_IPv4_Address_Request(Packet):
...
class PCO_DNS_Server_IPv4(Packet):
...
class PCO_IPv4_Link_MTU_Request(Packet):
...
class PCO_PasswordAuthentificationProtocol(Packet):
...
class PCO_PPP_Challenge(Packet):
    name = "PPP Password Authentification Protocol"
    fields_desc = [ByteField("Code", 0),
                   ByteField("Identifier", 0),
                   ShortField("length", 0),
...
class PCO_ChallengeHandshakeAuthenticationProtocol(Packet):
    name = "PCO Password Authentification Protocol"
    fields_desc = [ShortEnumField("type", None, PCO_PROTOCOL_TYPES),
                   ByteField("length", 0),
...
class IE_Bearer_QoS(gtp_v1.IE_Base):
...
class IE_ChargingID(gtp_v1.IE_Base):
    name = "IE Charging ID"
    fields_desc = [ByteEnumField("ietype", 1, IEType),
                   ShortField("length",  None),
                   BitField("CR_flag", 0, 4),
...
class IE_ChargingCharacteristics(gtp_v1.IE_Base):
    name = "IE Charging ID"
    fields_desc = [ByteEnumField("ietype", 1, IEType),
                   ShortField("length",  None),
                   BitField("CR_flag", 0, 4),
...
class IE_PDN_type(gtp_v1.IE_Base):
    name = "IE PDN Type"
    fields_desc = [ByteEnumField("ietype", 1, IEType),
                   ShortField("length",  None),
                   BitField("CR_flag", 0, 4),
...
class IE_UE_Timezone(gtp_v1.IE_Base):
...
class IE_APN_Restriction(gtp_v1.IE_Base):
...
class IE_SelectionMode(gtp_v1.IE_Base):
