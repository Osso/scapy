...
class ICMP(Packet):
    ...
    fields_desc = [ByteEnumField("type", 8, icmptypes),
                   ...
                   ConditionalField(
                       XShortField("seq", 0), lambda pkt:pkt.type in [0, 8, 13, 14, 15, 16, 17, 18]),
                   ConditionalField(
        ...
        ConditionalField(
                       ByteField("ptr", 0), lambda pkt:pkt.type == 12),
        ConditionalField(
...
def defrag(plist):
    ...
    for lst in frags.itervalues():
        ...
        p = lst[0]
        lastp = lst[-1]
        # first or last fragment missing
...
class TracerouteResult(SndRcvList):
    ...
    def make_graph(self, ASres=None, padding=0):
        ...
        # Find AS numbers
        ASN_query_list = dict.fromkeys(
            map(lambda x: x.rsplit(" ", 1)[0], ips)).keys()
        if ASres is None:
            ASNlist = []
        else:
...
#    pkt = IP(dst=target, id=RandShort(), options="\x22"*40)/UDP()/load
    pkt = IP(dst=target, id=RandShort(), options="\x00"*40, flags=1) / \
        UDP(sport=sport, dport=sport)/load
    s = conf.L3socket()
    intr = 0
    found = {}
    try:
