...
class DNS(Packet):
    ...
    fields_desc = [ShortField("id", 0),
                   BitField("qr", 0, 1),
                   BitEnumField(
                       "opcode", 0, 4, {0: "QUERY", 1: "IQUERY", 2: "STATUS"}),
                   BitField("aa", 0, 1),
                   BitField("tc", 0, 1),
                   BitField("rd", 0, 1),
                   BitField("ra", 0, 1),
...
def RRlist2bitmap(lst):
    ...
    for wb in xrange(min_window_blocks, max_window_blocks+1):
        ...
        for tmp in xrange(bytes):
            ...
            tmp_rrlist = filter(
                lambda x: 256*wb+8*tmp <= x and x < 256*wb+8*tmp+8, rrlist)
            if not tmp_rrlist == []:
...
dnssecclasses = [DNSRROPT, DNSRRRSIG, DNSRRDLV, DNSRRDNSKEY,
                 DNSRRNSEC, DNSRRDS, DNSRRNSEC3, DNSRRNSEC3PARAM]
