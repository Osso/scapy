...
class Net6(Gen):  # syntax ex. fec0::/126
    ...
    def __iter__(self):
        ...
        def parse_digit(a, netmask):
            ...
            return a
        self.parsed = map(
            lambda x, y: parse_digit(x, y), tmp, map(lambda x, nm=self.plen: x-nm, tuple))
...
class _IPv6GuessPayload:
    ...
    def default_payload_class(self, p):
        if self.nh == 58:  # ICMPv6
            ...
            if len(p) >= icmp6typesminhdrlen.get(t, sys.maxint):
...
class RouterAlert(Packet):  # RFC 2711 - IPv6 Hop-By-Hop Option
    ...
    # TODO : now that we have that option, we should do something in MLD class
    # that need it
...
def defragment6(pktlist):
    ...
    l = filter(lambda x: IPv6ExtHdrFragment in x, pktlist)
    if not l:
        return []
