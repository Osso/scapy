...
class L2CAP_ConnResp(Packet):
    ...
    fields_desc = [LEShortField("dcid", 0),
                   LEShortField("scid", 0),
                   LEShortEnumField(
                       "result", 0, ["no_info", "authen_pend", "author_pend"]),
...
class HCI_Cmd_Set_Event_Mask(Packet):
    name = "Set Event Mask"
    fields_desc = [
        StrFixedLenField("mask", "\xff\xff\xfb\xff\x07\xf8\xbf\x3d", 8)]
...
class BluetoothHCISocket(SuperSocket):
    ...
    def __init__(self, iface=0x10000, type=None):
        ...
        s.setsockopt(socket.SOL_HCI, socket.HCI_FILTER, struct.pack("IIIh2x", 0xffffffffL, 0xffffffffL, 0xffffffffL, 0))
        s.bind((iface,))
        self.ins = self.outs = s
#        s.connect((peer,0))
