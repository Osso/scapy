...
#        backcolor=makecol(0.376, 0.729, 0.525, 1.0)
        ...
        def make_dump(s, shift=0, y=0, col=None, bkcol=None, larg=16):
            ...
            c.stroke(
                make_frame(tlist), [col, pyx.deco.filled([bkcol]), pyx.style.linewidth.Thick])
            for txt in tlist:
                c.insert(txt)
        ...
        while t:
            ...
            y += 0.5
            pt = pyx.text.text(
                XSTART, (YTXT-y)*YMUL, r"\font\cmssfont=cmss10\cmssfont{%s}" % proto.name, [pyx.text.size.Large])
            y += 1
            ptbb = pt.bbox()
            ...
            for fname, fval, fdump in fields:
                ...
                if isinstance(fval, str):
                    ...
                    fval = ""
                vt = pyx.text.text(
                    XSTART+3, (YTXT-y)*YMUL, r"\font\cmssfont=cmss10\cmssfont{%s}" % tex_escape(fval))
                y += 1.0
                if fdump:
            ...
            try:
                ...
                if conf.debug_dissector:
                    if isinstance(cls, type) and issubclass(cls, Packet):
                        ...
                        log_runtime.error(
                            "%s.guess_payload_class() returned [%s]" % (self.__class__.__name__, repr(cls)))
                    if cls is not None:
                        raise
    ...
    def show(self, indent=3, lvl="", label_lvl=""):
        ...
        self.payload.show(
            indent=indent, lvl=lvl+(" "*indent*self.show_indent), label_lvl=label_lvl)
