...
class PacketListField(PacketField):
    ...
    def build_python(self, pkt, x):
        ...
        if x is None:
            ...
            els = [self.build_python(pkt, el) for el in x]
            return ct.punct('[') + ", ".join(els) + ct.punct(']')
        elif isinstance(x, Packet):
            return x.build_python()
        else:
            return x.build_python(pkt, x)
...
class _EnumField(Field):
    ...
    def build_python_one(self, pkt, x):
        if self not in conf.noenum and not isinstance(x,VolatileValue) and x in self.i2s:
            return repr(self.i2s[x])
        return repr(x)

    def build_python(self, pkt, x):
        if type(x) is list:
            return map(lambda z, pkt=pkt: self.build_python_one(pkt,z), x)
        else:
            return self.build_python_one(pkt, x)
