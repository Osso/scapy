...
pcap_handler = CFUNCTYPE(
    None, POINTER(c_ubyte), POINTER(pcap_pkthdr), POINTER(c_ubyte))
...
pcap_lookupnet.argtypes = [
    STRING, POINTER(bpf_u_int32), POINTER(bpf_u_int32), STRING]
...
pcap_next_ex.argtypes = [
    POINTER(pcap_t), POINTER(POINTER(pcap_pkthdr)), POINTER(POINTER(u_char))]
...
pcap_dump.argtypes = [
    POINTER(pcap_dumper_t), POINTER(pcap_pkthdr), POINTER(u_char)]
...
pcap_compile.argtypes = [
    POINTER(pcap_t), POINTER(bpf_program), STRING, c_int, bpf_u_int32]
...
pcap_compile_nopcap.argtypes = [
    c_int, c_int, POINTER(bpf_program), STRING, c_int, bpf_u_int32]
...
if WIN32:
    ...
    pcap_findalldevs_ex.argtypes = [
        STRING, POINTER(pcap_rmtauth), POINTER(POINTER(pcap_if_t)), STRING]
    ...
    pcap_parsesrcstr.argtypes = [
        STRING, POINTER(c_int), STRING, STRING, STRING, STRING]
    ...
    pcap_open.argtypes = [
        STRING, c_int, c_int, c_int, POINTER(pcap_rmtauth), STRING]
