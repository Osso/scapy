...
if conf.use_winpcapy:
    ...
    class _PcapWrapper_pypcap:
        ...
        def next(self):
            c = pcap_next_ex(
                self.pcap, byref(self.header), byref(self.pkt_data))
            if not c > 0:
                return
    ...
    if conf.use_pcap:
        ...
        if hasattr(pcap, "pcap"):  # python-pypcap
            class _PcapWrapper_pypcap:
                ...
                def next(self):
                    ...
                    return ts, str(pkt)
            open_pcap = lambda * \
                args, **kargs: _PcapWrapper_pypcap(*args, **kargs)
            ...
            class _PcapWrapper_libpcap:
                ...
                def __del__(self):
                    ...
                    os.close(fd)
            open_pcap = lambda * \
                args, **kargs: _PcapWrapper_libpcap(*args, **kargs)
            ...
            class _PcapWrapper_pcapy:
                ...
                def __del__(self):
                    warning(
                        "__del__: don't know how to close the file descriptor. Bugs ahead ! Please report this bug.")
            open_pcap = lambda * \
                args, **kargs: _PcapWrapper_pcapy(*args, **kargs)
        ...
        class L2pcapListenSocket(SuperSocket):
            ...
            def __init__(self, iface=None, type=ETH_P_ALL, promisc=None, filter=None):
                ...
                try:
                    ioctl(
                        self.ins.fileno(), BIOCIMMEDIATE, struct.pack("I", 1))
                except:
                    pass
                # Do not apply any filter if Ethernet type is given
                if type == ETH_P_ALL:
    ...
    try:
        ...
        def get_if_raw_hwaddr(iff):
            ...
            try:
                ...
                raise Scapy_Exception(
                    "Error in attempting to get hw address for interface [%s]" % iff)
